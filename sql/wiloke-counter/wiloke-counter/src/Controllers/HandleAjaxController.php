...
use Exception;
use WilokeCounter\Share\TraitHandleCustomPost;
use WilokeCounter\Share\TraitHandleCustomProduct;
use WilokeCounter\Share\TraitHandleQuery;
...
class HandleAjaxController
{
	...
	{
		add_action('wp_ajax_' . WILOKE_WILOKECOUNTER_NAMESPACE .
		...
		add_action('wp_ajax_nopriv_' . WILOKE_WILOKECOUNTER_NAMESPACE .
			'_custom_wil_product_tags',
			[$this, 'ajaxCustomWilProductTags']);
	...
	public function ajaxCheckListProductsIsWishList()
	{
		try {
			...
			wp_send_json([
				'message' => esc_html__("found it", WILOKE_WILOKECOUNTER_NAMESPACE),
				'status'  => esc_html__('success', WILOKE_WILOKECOUNTER_NAMESPACE),
				'items'   => $aResponseData
			], 200);
			die();
		}
			...
			wp_send_json([
				...
				'status'  => esc_html__('error', WILOKE_WILOKECOUNTER_NAMESPACE),
			], $exception->getCode());
			die();
		}
	}
	...
	public function ajaxFindTerms()
	{
		if (!isset($_GET['taxonomy']) || empty($_GET['taxonomy'])) {
			wp_send_json([
				'message' => esc_html__('You must select Taxonomy field first.', 'wiloke-counter'),
				'status'  => esc_html__('error', WILOKE_WILOKECOUNTER_NAMESPACE),
			], 400);
		}
